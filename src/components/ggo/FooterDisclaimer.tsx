...
export function FooterDisclaimer({ className = "", compact = true }: FooterDisclaimerProps) {
  return (
    <footer
    ...
    >
      <div className="max-w-7xl mx-auto px-6 py-3 space-y-2">
        {/* Compact: single line disclaimer + copyright + links */}
        {compact ? (
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-ggo-text-muted" style={{ fontSize: '11px', lineHeight: '1.5' }}>
            <span className="flex items-center gap-1.5">
              <AlertTriangle className="w-3 h-3 text-amber-500 flex-shrink-0" aria-hidden="true" />
              <span>Educational only — always follow your clinician&apos;s instructions.</span>
            </span>
            <span aria-hidden="true">&middot;</span>
            <span>&copy; {new Date().getFullYear()} GGO Med<sup>&reg;</sup> Ltd</span>
            <span aria-hidden="true">&middot;</span>
            <Link href="/privacy" className="text-ggo-teal hover:underline underline-offset-2">Privacy</Link>
            <span aria-hidden="true">&middot;</span>
            <Link href="/settings" className="text-ggo-teal hover:underline underline-offset-2">Settings</Link>
          </div>
        ) : (
          <>
            {/* Full: Tier 1 — Safety Disclaimer */}
            ...
            {/* Full: Tier 2 — Prototype Declaration */}
            <div className="flex items-start gap-2.5 text-ggo-text-muted">
